...
def test_build_and_specification(and_specification):
    assert FirestoreSpecificationBuilder.build(and_specification) == [("id", "==", 1), ("name", "==", "test")]


def test_build_contains_specification(contains_specification):
    assert FirestoreSpecificationBuilder.build(contains_specification) == ("field", "array-contains", "test")


def test_build_in_specification(in_specification):
    assert FirestoreSpecificationBuilder.build(in_specification) == ("field", "in", [1, 2, 3])
...
def test_build_less_than_specification(less_than_specification):
    assert FirestoreSpecificationBuilder.build(less_than_specification) == ("id", "<", 1)


def test_build_less_than_equal_specification(less_than_equal_specification):
    assert FirestoreSpecificationBuilder.build(less_than_equal_specification) == ("id", "<=", 1)


def test_build_greater_than_specification(greater_than_specification):
    assert FirestoreSpecificationBuilder.build(greater_than_specification) == ("id", ">", 1)


def test_build_greater_than_equal_specification(greater_than_equal_specification):
    assert FirestoreSpecificationBuilder.build(greater_than_equal_specification) == ("id", ">=", 1)


def test_build_regex_string_match_specification(regex_string_match_specification):
    assert FirestoreSpecificationBuilder.build(regex_string_match_specification) == ('id', 'array-contains', 'abc')
...
def test_build_dict_specification(dict_specification):
    assert FirestoreSpecificationBuilder.build(dict_specification) == [("id", "==", 1), ("test", "==", 2)]
