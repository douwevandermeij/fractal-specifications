...
def test_build_or_specification(or_specification):
    assert DjangoOrmSpecificationBuilder.build(or_specification) == Q(id=1) | Q(name="test")


def test_build_and_specification(and_specification):
    assert DjangoOrmSpecificationBuilder.build(and_specification) == Q(id=1, name="test")
...
def test_build_in_specification(in_specification):
    assert DjangoOrmSpecificationBuilder.build(in_specification) == Q(field__in=[1, 2, 3])


def test_build_in_empty_specification(in_empty_specification):
    assert DjangoOrmSpecificationBuilder.build(in_empty_specification) == Q(field__in=[])
...
def test_build_less_than_equal_specification(less_than_equal_specification):
    assert DjangoOrmSpecificationBuilder.build(less_than_equal_specification) == Q(id__lte=1)


def test_build_greater_than_specification(greater_than_specification):
    assert DjangoOrmSpecificationBuilder.build(greater_than_specification) == Q(id__gt=1)


def test_build_greater_than_equal_specification(greater_than_equal_specification):
    assert DjangoOrmSpecificationBuilder.build(greater_than_equal_specification) == Q(id__gte=1)


def test_build_regex_string_match_specification(regex_string_match_specification):
    assert DjangoOrmSpecificationBuilder.build(regex_string_match_specification) == Q(id__regex=".*abc.*")


def test_build_is_none_specification(is_none_specification):
    assert DjangoOrmSpecificationBuilder.build(is_none_specification) == Q(field__isnull=True)
