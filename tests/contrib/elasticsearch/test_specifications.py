...
def test_build_equals_specification(equals_specification):
    assert ElasticSpecificationBuilder.build(equals_specification) == {"match": {"id.keyword": 1}}
...
def test_build_in_specification(in_specification):
    assert ElasticSpecificationBuilder.build(in_specification) == {"query_string": {"default_field": "field", "query": [1, 2, 3]}}


def test_build_in_empty_specification(in_empty_specification):
    assert ElasticSpecificationBuilder.build(in_empty_specification) == {"query_string": {"default_field": "field", "query": []}}


def test_build_less_than_specification(less_than_specification):
    assert ElasticSpecificationBuilder.build(less_than_specification) == {"bool": {"filter": [{"range": {"id": {"lt": 1}}}]}}


def test_build_less_than_equal_specification(less_than_equal_specification):
    assert ElasticSpecificationBuilder.build(less_than_equal_specification) == {"bool": {"filter": [{"range": {"id": {"lte": 1}}}]}}


def test_build_greater_than_specification(greater_than_specification):
    assert ElasticSpecificationBuilder.build(greater_than_specification) == {"bool": {"filter": [{"range": {"id": {"gt": 1}}}]}}


def test_build_greater_than_equal_specification(greater_than_equal_specification):
    assert ElasticSpecificationBuilder.build(greater_than_equal_specification) == {"bool": {"filter": [{"range": {"id": {"gte": 1}}}]}}
