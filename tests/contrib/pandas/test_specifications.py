...
def test_build_equals_specification(equals_specification):
    assert PandasSpecificationBuilder.build(equals_specification)(df).to_dict() == df[df.id == 1].to_dict()


def test_build_or_specification(or_specification):
    assert PandasSpecificationBuilder.build(or_specification)(df).to_dict() == (df[(df.id == 1) | (df.name == "test")]).to_dict()


def test_build_and_specification(and_specification):
    assert PandasSpecificationBuilder.build(and_specification)(df).to_dict() == (df[(df.id == 1) & (df.name == "test")]).to_dict()
...
def test_build_in_specification(in_specification):
    assert PandasSpecificationBuilder.build(in_specification)(df).to_dict() == df[df.field.isin([1, 2, 3])].to_dict()


def test_build_in_empty_specification(in_empty_specification):
    assert PandasSpecificationBuilder.build(in_empty_specification)(df).to_dict() == df[df.field.isin([])].to_dict()


def test_build_less_than_specification(less_than_specification):
    assert PandasSpecificationBuilder.build(less_than_specification)(df).to_dict() == df[df.id < 1].to_dict()


def test_build_less_than_equal_specification(less_than_equal_specification):
    assert PandasSpecificationBuilder.build(less_than_equal_specification)(df).to_dict() == df[df.id <= 1].to_dict()


def test_build_greater_than_specification(greater_than_specification):
    assert PandasSpecificationBuilder.build(greater_than_specification)(df).to_dict() == df[df.id > 1].to_dict()


def test_build_greater_than_equal_specification(greater_than_equal_specification):
    assert PandasSpecificationBuilder.build(greater_than_equal_specification)(df).to_dict() == df[df.id >= 1].to_dict()
...
def test_build_is_none_specification(is_none_specification):
    assert PandasSpecificationBuilder.build(is_none_specification)(df).to_dict() == {'field': {}, 'id': {}, 'name': {}}
...
def test_build_with_index_equals_specification(equals_specification):
    assert PandasIndexSpecificationBuilder.build(equals_specification)(dfi).to_dict() == dfi[dfi.index.get_level_values("id") == 1].to_dict()


def test_build_with_index_or_specification(or_specification):
    assert PandasIndexSpecificationBuilder.build(or_specification)(dfi).to_dict() == (
            dfi[
...
def test_build_with_index_and_specification(and_specification):
    assert PandasIndexSpecificationBuilder.build(and_specification)(dfi).to_dict() == (
            dfi[
                (dfi.index.get_level_values("id") == 1)
                & (dfi.index.get_level_values("name") == "test")
            ]
...
def test_build_with_index_in_specification(in_specification):
    assert PandasIndexSpecificationBuilder.build(in_specification)(dfi).to_dict() == dfi[dfi.index.get_level_values("field").isin([1, 2, 3])].to_dict()


def test_build_with_index_in_empty_specification(in_empty_specification):
    assert PandasIndexSpecificationBuilder.build(in_empty_specification)(dfi).to_dict() == dfi[dfi.index.get_level_values("field").isin([])].to_dict()


def test_build_with_index_less_than_specification(less_than_specification):
    assert PandasIndexSpecificationBuilder.build(less_than_specification)(dfi).to_dict() == dfi[dfi.index.get_level_values("id") < 1].to_dict()


def test_build_with_index_less_than_equal_specification(less_than_equal_specification):
    assert PandasIndexSpecificationBuilder.build(less_than_equal_specification)(dfi).to_dict() == dfi[dfi.index.get_level_values("id") <= 1].to_dict()


def test_build_with_index_greater_than_specification(greater_than_specification):
    assert PandasIndexSpecificationBuilder.build(greater_than_specification)(dfi).to_dict() == dfi[dfi.index.get_level_values("id") > 1].to_dict()


def test_build_with_index_greater_than_equal_specification(greater_than_equal_specification):
    assert PandasIndexSpecificationBuilder.build(greater_than_equal_specification)(dfi).to_dict() == dfi[dfi.index.get_level_values("id") >= 1].to_dict()


def test_build_with_index_regex_string_match_specification(regex_string_match_specification):
    with pytest.raises(SpecificationNotMappedToPandas):
...
def test_build_with_index_is_none_specification(is_none_specification):
    assert PandasIndexSpecificationBuilder.build(is_none_specification)(dfi).to_dict() == {}
